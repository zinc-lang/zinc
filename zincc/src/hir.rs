pub mod gen {
    ...
    pub fn stage1<'s>(
        sd: &'s SharedData<'s>,
        root: &ast::Root,
    ) -> IndexVec<stage1::ScopeDesc, stage1::ScopeDescId> {
    ...
    pub mod stage2 {
        ...
        impl<'s> Stage2<'s> {
            fn resolve_ty_path(&mut self, path: &ast::Path) -> hir::TyPathResoution {
                if path.segments.len() == 1 {
                    if let Some(ast::PathSegment::Ident(idx)) = path.segments.get(0) {
                        ...
                        let prim_ty = match str {
                            "sint" => hir::PrimTy::Sint(hir::IntSize::Unspecified),
                            "uint" => hir::PrimTy::Uint(hir::IntSize::Unspecified),
                            "sintptr" => hir::PrimTy::Sint(hir::IntSize::PtrSized),
                            "uintptr" => hir::PrimTy::Uint(hir::IntSize::PtrSized),
                            "bool" => hir::PrimTy::Bool,
                            "void" => hir::PrimTy::Void,
                            // if it starts with 's' and the rest of the string is a number
                            _ if str.starts_with('s') && chk_int_num(&str[1..]) => {
                                hir::PrimTy::Sint(hir::IntSize::BitSized(str[1..].parse().unwrap()))
                            }
                            // if it starts with 'u' and the rest of the string is a number
                            _ if str.starts_with('u') && chk_int_num(&str[1..]) => {
                                hir::PrimTy::Uint(hir::IntSize::BitSized(str[1..].parse().unwrap()))
                            }
                            _ => todo!(),
                        };
                        hir::TyPathResoution::PrimTy(prim_ty)
                    } else {
                        todo!()
                    }
                } else {
